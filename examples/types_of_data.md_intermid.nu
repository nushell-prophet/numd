...
try {{ ...$data, hobby: cricket }} catch {|e| $e} | print $in
...
try {[bell book candle] | where ($it =~ 'b')} catch {|e| $e} | print $in
...
try {[...$x 3 ...(4..7 | take 2)]} catch {|e| $e} | print $in
...
print '```
```nudoc-output'
git checkout featurebranch | null | print $in
